...
def generate_audio_file(diagnosis, medications, treatment_plan):
...
def generate_pdf_report(name, age, gender, diagnosis, medications, treatment_plan):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(200, 10, txt=f"Predicted Diagnosis: {diagnosis}", ln=True, align='L')
    pdf.cell(200, 10, txt=f"Recommended Medications: {medications}", ln=True, align='L')
    pdf.cell(200, 10, txt=f"Suggested Treatment Plan: {treatment_plan}", ln=True, align='L')
